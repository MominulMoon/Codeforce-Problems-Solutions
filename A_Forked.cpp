...
#define FOR(i, j, n) for(long i = j; i < n; i++)
#define trav(i, a) for(auto &i: a) cin >> i;
#define all(x) x.begin(), x.end()
#define reverse_sort(v) sort(all(v),greater<long>())
...
void solve(){
    ...
    FOR(i,0,4)
    {
        king.insert({ka+dx[i]*a,kb+dy[i]*b});
        king.insert({ka+dx[i]*b,kb+dy[i]*a});

        queen.insert({qa+dx[i]*a,qb+dy[i]*b});
        queen.insert({qa+dx[i]*b,qb+dy[i]*a});
    }
    ll ans=0;
    for(auto i:king)
